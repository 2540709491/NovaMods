...
namespace ConfigurationManager
{
    ...
    /// </summary>
    [BepInPlugin(GUID, "MOD配置管理", Constants.Version)]
    [Browsable(false)]
    public class ConfigurationManager : BaseUnityPlugin,IModConfig
    {
        ...
        private bool isModSearch = false;
        private Dictionary<string, string> ModSearchResult = new Dictionary<string, string>();
        /// <inheritdoc />
        ...
        private void OnGUI()
        {
            if (DisplayingWindow)
            {
                ...
                var newRect = GUILayout.Window(WindowId, SettingWindowRect, (GUI.WindowFunction)SettingsWindow, "激射超新星模组配置");
        ...
        private void SettingsWindow(int id)
        {
            ...
            GUILayout.BeginVertical();
            {
                ...
                {
                    GUILayout.Space(10);
                    GUILayout.Label("没有可用选项的插件：" + _modsWithoutSettings);
                }
                else
                {
        ...
        {
            var tip = !_tipsPluginHeaderWasClicked ? "提示：点击插件名称以展开，点击设置和组名称以查看其描述。" :
                !_tipsWindowWasMoved ? "你可以拖动此窗口来移动它。当你与游戏交互时，它将保持打开状态。" : null;
        ...
        private void DrawWindowHeader()
        {
            GUILayout.BeginHorizontal(GUI.skin.box);
            {
                ...
                var newVal = GUILayout.Toggle(_showSettings.Value, "常规设置");
                ...
                newVal = GUILayout.Toggle(_showKeybinds.Value, "键盘快捷键");
                ...
                //进阶设置隐藏(定制)
                // newVal = GUILayout.Toggle(_showAdvanced.Value, "进阶设置");
                ...
                newVal = GUILayout.Toggle(_showDebug, "调试信息");
            ...
            GUILayout.BeginHorizontal(GUI.skin.box);
            {
                ...
                if (GUILayout.Button(_pluginConfigCollapsedDefault.Value ? "展开全部" : "收起全部", GUILayout.ExpandWidth(false)))
                {
        ...
        private void DrawSinglePlugin(PluginSettingsData plugin)
        {
            ...
            if (!isModSearch)
            {
                bool _isDisabledRank = isDisabledRank($"{plugin.Info.Name.TrimStart('!')} {plugin.Info.Version}");
                string DRmessage1 = _isDisabledRank ? "禁用排行榜" : "允许排行榜";
                ModSearchResult.Add($"{plugin.Info.Name.TrimStart('!')} {plugin.Info.Version}",DRmessage1);  
            }
            ModSearchResult.TryGetValue($"{plugin.Info.Name.TrimStart('!')} {plugin.Info.Version}",out string DRmessage);
            ...
            var categoryHeader = _showDebug ?
                new GUIContent($"{plugin.Info.Name.TrimStart('!')} {plugin.Info.Version} ({DRmessage})", null, "GUID: " + plugin.Info.GUID) :
                new GUIContent($"{plugin.Info.Name.TrimStart('!')} {plugin.Info.Version} ({DRmessage})");
        ...
        private void DrawSingleSetting(SettingEntryBase setting)
        {
            GUILayout.BeginHorizontal();
            {
                ...
                {
                    Logger.Log(LogLevel.Error, $"绘制设置失败: {setting.DispName} - {ex}");
                    GUILayout.Label("无法绘制此属性,查看日志以了解细节.");
                }
            }
            GUILayout.EndHorizontal();
        }
        ...
        private bool isDisabledRank(string modName)
        {

            List<ModProxy> modSettingList = ModMgr.ModList;
            foreach (ModProxy modSeting in modSettingList)
            {
                Debug.Log($"[CM Debug] {modSeting.Name} - {modName}");
                if (modSeting.Name==modName)
                {
                    return modSeting.Config.DisableRank;
                }   
            }
            return true;
        }
        ...
        private void Start()
        {
            ...
            try { Config.Save(); }
            catch (IOException ex) { Logger.Log(LogLevel.Message | LogLevel.Warning, "警告:无法写入配置目录，请预期会出现问题！\n错误信息:" + ex.Message); }
            catch (UnauthorizedAccessException ex) { Logger.Log(LogLevel.Message | LogLevel.Warning, "警告：没有权限写入配置目录，请预期会出现问题！\n错误信息：" + ex.Message); }
        }
